...
function resolveModel(provider, modelFromConfig) {
    ...
    const valid = exports.VALID_MODELS[provider];
    if (!valid) {
    ...
    const fallback = exports.DEFAULT_MODEL[provider];
    console.warn(`[LLM] Model "${effectiveModel}" is not valid for provider "${provider}". ` +
